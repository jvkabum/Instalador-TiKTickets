...
backend_db_create() {
  ...
  docker run --name redis-tiktickets \
                -e TZ="America/Sao_Paulo" \
                -p 6379:6379 \
                --restart=always \
...
# Limitar Uso do tiktickets Usuario e Conexões
USER_LIMIT=99
...
git_update() {
  print_banner
  printf "${WHITE} 💻 Atualizando o tiktickets do git...${GRAY_LIGHT}"
  printf "\n\n"
